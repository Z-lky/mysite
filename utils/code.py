...
def gene_code():
    ...
    path = os.path.join(os.path.dirname(__file__), 'Arial.ttf')
    print(os.path.exists(path), '0-----path正常获取------')
    font = ImageFont.truetype(path, 36)  # 验证码的字体
    draw = ImageDraw.Draw(image)  # 创建画笔
    text = gene_text()  # 生成字符串
    font_width, font_height = draw.textsize(text, font=font)  # 计算字符串的宽度和高度
